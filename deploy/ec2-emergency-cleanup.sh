...
if [ -d /app/.git ]; then
  git -C /app reflog expire --expire=now --all 2>/dev/null
  git -C /app gc --prune=now 2>/dev/null
fi
...
if [ "${avail_kb:-0}" -lt 262144 ] 2>/dev/null; then
  echo "CRITICAL: less than 256 MB free — expand EBS volume in AWS Console (16 GB+ recommended)." >&2
  exit 1
fi
