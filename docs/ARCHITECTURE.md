...
*Auto-maintained by Stop hook (`/.claude/hooks/update-architecture.sh`). Last updated: 2026-07-01 session 12 (body updated to reflect session 12 structural changes: (1) Android `RtmpCredentialsStore.kt` now hosts two extension functions on `OverlayLayoutPrefs` that define the Android adapter layer: `toEngineLayout(sponsorLogoUrls)` — full KMP→`StreamCameraEngine.OverlayLayout` mapping carrying all sponsor, scroll, watermark, and board-scale fields — and `bottomMarginPx(frameHeightPx)` — converts `bottomMargin` (raw double) to physical pixels via `(bottomMargin / 720f).coerceIn(0f, 0.2f) * frameHeightPx`; iOS equivalent is `OverlayLayoutPrefs.toEngineLayout()` defined in `Models.swift`; (2) iOS `StreamRecap` gains `durationText` computed property — formats `durationSeconds` as `m:ss` for sub-hour broadcasts and `h:mm:ss` once crossing one hour; (3) `OverlaySpriteLayoutTest` expanded to 10 test cases: `computePosition` bottom-placement, bottom-flush assertion (`posY + scaleY == 100` when `bottomMarginFraction == 0`), margin-lift direction, bounds clamping, legacy-coord regression guard; `fitScale` aspect preservation at multipliers 0.4/0.6/0.8/1.0, native-size passthrough when bitmap fits, portrait-overflow proportional shrink, range clamping; `shouldForceTransparentBackground` Compose-host detection. Prior session 11 changes: (1) iOS `StudioMenuSheet` gains "Share watch link" `ShareLink` action; (2) iOS `OverlaySheet` reaches full parity with Android `StudioSheets.kt`; (3) iOS `Models.swift` defines `SponsorLayoutMode`, `SponsorScrollDirection`, `SponsorDisplayMode` as Swift caseless enums; (4) iOS `StreamCameraEngine` branches on `SponsorScrollDirection.isHorizontal()`/`isVertical()`; (5) Android `BroadcastCameraUi` refactored: shared private composables (`ToolButtons`, `QuickToggles`, `PauseReveal`, `FocusReticle`, `StudioTopBar`, `StudioStatusMessages`, `ShutterLabel`) consumed by both `PortraitControls` and `LandscapeControls`; (6) `StreamRtmpPlugin.swift` `overlayLayout(from:)` carries only basic layout params — sponsor fields not forwarded through Flutter method channel; (7) `OverlaySpriteLayoutTest` added.)*
