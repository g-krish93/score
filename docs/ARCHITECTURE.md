...
*Auto-maintained by Stop hook (`/.claude/hooks/update-architecture.sh`). Last updated: 2026-06-30 (public live-score page + SSE; public club page; SEO infrastructure; password reset flow; iOS OTA install; viewer-stat columns; ui_theme; scoring dual-write flags; stream slot cap; Sponsor time bounds; cricheroes.com + cricheroes.in dual-host support via CRICHEROES_HOSTS + _is_cricheroes_host; canonicalize_cricheroes_scrape_url returns "" for invalid inputs; fixture Weekly-tab priority; normalize_cricheroes_team_root; relay_source_to_provider; relay_source="native" for club-owned tournament fixtures; tournament management dashboard routes + round-robin generation; public /t/<slug> tournament page; compute_points_table NRR standings).*
...
| `models_cricrelay.py` | SQLAlchemy models: `Organization` (+ `ui_theme` original/light/dark, branding fields), `ClubUser`, `RelayMatch`, `StreamSession` (+ `started_by_user_id`, `match_label`, `peak_viewers`, `viewer_sample_sum/count`, `final_score_json`, `vod_url`), `Sponsor` (+ `active_from`/`active_to` time bounds), `Tournament`, `Team`, `Player`, `Fixture`; `RELAY_PROVIDERS` dispatch table; `CRICHEROES_HOSTS` (`{"cricheroes.com", "cricheroes.in"}`); `_is_cricheroes_host()` (validates netloc against CRICHEROES_HOSTS, including subdomains); URL canonicalization helpers (`canonicalize_play_cricket_scrape_url`, `canonicalize_cricheroes_scrape_url` — returns `""` for any non-CricHeroes URL, `normalize_cricheroes_team_root`); `relay_source_to_provider()` (maps `relay_source` string → provider key); `resolve_provider_callable()` lazy-import; `provider_poll_interval_sec()` |
