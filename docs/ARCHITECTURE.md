...
*Auto-maintained by Stop hook (`/.claude/hooks/update-architecture.sh`). Last updated: 2026-07-01 session 7 (re-validated all streaming pipeline files — no structural changes from session 6; confirmed: Android `BroadcastCameraUi` fires haptic feedback (`LongPress`) on streaming start; Android `StudioViewModel.onStudioHidden()` resets `previewSurfaceBound` and status message only when not streaming; iOS `StudioViewModel.load()` fetches matchDay + overlayPrefs + scoringConfig in parallel then resolves StreamMatch row + live platform status in `loadStudioExtras()`; iOS `syncOverlay()` resolves sponsor logo URLs before calling `StreamCameraEngine.updateOverlay()`; Android `StreamCameraEngine.visibleSponsorUrls()` returns at most 1 URL in carousel mode; iOS `StreamCameraEngine` sponsor scroll/layout handles all 5 display modes; KMP `OverlayLayoutPrefs.resolveSponsorLogoUrls()` falls back to all active sponsors for multi/carousel when no explicit IDs are selected; server `models_cricrelay.py` unchanged — all documented facts confirmed accurate vs. current code).*

*Confirmed in current code: iOS Go Live countdown is 3→1; Android `StreamRecap` (`durationSeconds`, `destinationLabel`, `watchUrl`) has no `title` field — iOS `StreamRecap` does (`title`, `destinationLabel`, `durationSeconds: Int`, `watchUrl`); `StreamOverlayPolicy.refreshMode` accepts `hasPreviewListener` for API compat but does not use it in the decision branch (result is always `None`/`StreamRefresh`/`PreviewGlRefresh` based on `isStreaming` and `overlayUrlBlank` only); Android `StudioViewModel` live-timer increments a plain counter (`elapsed++`), not wall-clock — iOS `StudioViewModel` stamps `liveStartedAt: Date?` and computes `Date().timeIntervalSince(liveStartedAt)` so background throttling can't cause drift; poll cadences — Android matchDay=8 s, remote-commands=1.5 s; iOS matchDay=5 s, remote-commands=1.5 s; `OverlayLayoutPrefs.sanitizeTheme()` recognises HTML class names `{"classic","neon","minimal","compact","ai","stadium"}` and maps `"dark"→"classic"` (OverlaySheet display labels: Broadcast→classic, AI Neural→ai); `RELAY_PROVIDERS` contains only `"play_cricket"` and `"cricheroes"` — `relay_source="native"` matches have no provider entry and no scraper (manual-only); `relay_source_to_provider()` maps `pcs_ble→None`, `cricheroes→cricheroes`, anything else→`play_cricket`; iOS `StreamCameraEngine.buildStandbyImage()` composes a Floodlight-branded full-frame slate (Ink background `#0A0E15`, Gold `#FFC233` title "Screen locked", white subtitle "Back shortly — commentary continues") shown while the app is backgrounded during a live stream; iOS `StreamCameraEngine` also has `pauseBlackObject: ImageScreenObject?` + `showPauseBlackOverlay()` / `hidePauseBlackOverlay()` to cover the screen with full-frame black during a paused broadcast (mirrors Android `BlackFilterRender`); iOS `syncEncoderForGoLive(width:height:fps:bitrate:)` re-prepares the preview if the capture orientation drifted between Studio open and the Go Live tap (parity with Android's startStreamOnMain rotation re-sync); `CRICHEROES_HOSTS = {"cricheroes.com", "cricheroes.in"}` + `_is_cricheroes_host()` are the single source of truth for CricHeroes host validation in `models_cricrelay.py`; `canonicalize_cricheroes_scrape_url` returns `""` for any non-CricHeroes URL; `_cricheroes_match_id_from_url` uses two-pass regex (`/(?:scorecard|individual)/(\d+)` then `/scorecard/(\d+)/`) and normalises `/individual/<id>/live` → `/scorecard/<id>/live`; `OverlayLayoutPrefs` carries `activeSponsorIds: List<String>` (multi-select) alongside legacy `activeSponsorId`; `effectiveSponsorIds()` + `resolveSponsorLogoUrls()` + `mergeSponsorPatch()` + `sponsorPatchJson()` on all three platforms (KMP, iOS, Android extension); KMP `OverlayLayoutPrefs` also exposes `boardDisplayScaleX()` / `boardDisplayScaleY()` (display scale of slider vs reference board size), `clampedWidthFraction()` / `clampedHeightFraction()` (coerce to safe range), and `effectiveFontScale()` (user font slider, separate from board W/H scale) for the OverlaySheet UI; `OverlayLayoutPrefs` on all three platforms (KMP `Models.kt`, iOS `Models.swift`, Android extension in `RtmpCredentialsStore.kt`) carries `videoStabilization` and `keepScreenOn` as persisted camera-config fields — bootstrapped from the server at Studio load and written back via `saveOverlay()`/`updateOverlayPrefs()`; `CompanionSession` struct in iOS `Models.swift` (companionToken + matchSlug) for future companion-device auth; `previewOverlayPrefs()` / `previewOverlay()` on both platforms pushes overlay changes to the GL preview without persisting to the server; `revertOverlayPreview()` restores the last saved state on sheet dismiss; iOS `cameraConfigQueue` serial `DispatchQueue` serialises all `AVCaptureDevice.lockForConfiguration` calls; `BroadcastCameraUi` `onShare` callback replaces the Menu button (animated crossfade) when non-null and streaming; `StreamOverlayPolicyTest` covers all `refreshMode` branches + both `shouldAttachGlOverlay*` helpers; Android `StudioViewModel` lifecycle hooks: `onStudioVisible()` and `onConfigurationChanged()` both call `streamController.refreshNativePreview()` (surface/config rebind); `onDeviceOrientationChanged(surfaceRotationDegrees: Int)` relays sensor rotation to `streamController.setDeviceOrientation()` so the encoder re-prepares at the new orientation before Go Live.*
...
- `Models.swift` — shared domain structs: `StreamMatch`, `OverlayLayoutPrefs` (full sponsor prefs: `activeSponsorId`, `activeSponsorIds: [String]` (multi-select; `effectiveSponsorIds()` collapses both), `resolveSponsorLogoUrls(from:)` resolves logo URLs by ID; `sponsorLayoutMode`, `sponsorCarouselIntervalSec`, `sponsorDisplayMode`, `sponsorPositionX/Y`, `sponsorSizeScale`, `sponsorOpacity`, `sponsorScrollSpeed`; `mergeSponsorPatch(_:)` for remote companion patch; `toEngineLayout(sponsorLogoUrls:)` maps to `StreamCameraEngine.OverlayLayout`), `ScoringConfig`, `GoLiveResult`, `StreamRecap`, `PlatformStatus`, `RtmpCredentials`, `RemoteCommand`, `RemoteCompanionContext`, `PairRemoteResult`, `CompanionSession` (companionToken + matchSlug — for future companion-device auth), `MatchDayStatus`; `SponsorLayoutMode` (single/multi/carousel) and `SponsorDisplayMode` (static/scroll_top/scroll_bottom/scroll_above_board/scroll_below_board) enums
...
- `onShare: (() -> Unit)?` callback — when non-null and streaming, the top-bar Menu button is replaced by a Share button (animated crossfade).
...
| Domain models | `StreamMatch`, `OverlayLayoutPrefs` (full sponsor prefs + `activeSponsorIds: List<String>` multi-select + `effectiveSponsorIds()` + `resolveSponsorLogoUrls()` (ID-based lookup → active-sponsor fallback) + `SponsorLayoutMode`/`SponsorDisplayMode` objects + `mergeSponsorPatch()`/`sponsorPatchJson()`; board-scale helpers: `boardDisplayScaleX()` / `boardDisplayScaleY()` (slider vs reference board fraction), `clampedWidthFraction()` / `clampedHeightFraction()` (coerce to safe range), `effectiveFontScale()` (user font slider, independent of board W/H)), `ScoringConfig`, `MatchDayStatus`, `PlatformStatus`, `GoLiveResult`, `FixturesResponse`, `FixtureItem`, `Sponsor`, `RemoteCommand` (with `mergeSponsorInto()`), `RemoteCompanionContext`, `PairRemoteResult`, `BroadcastStatus` |
