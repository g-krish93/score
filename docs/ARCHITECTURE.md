...
*Auto-maintained by Stop hook (`/.claude/hooks/update-architecture.sh`). Last updated: 2026-07-01 session 9 (structural changes: (1) Arrange mode added on Android — `StudioUiState` gains `arrangeMode`, `arrangeTarget` (`Board`/`Sponsor`), `arrangeDraft`; `StudioViewModel` gains `enterArrangeMode()` / `cancelArrangeMode()` / `commitArrangeMode()` / `pinchBoard()` / `dragArrange()` — gestures push to GL preview without server persist; `commitArrangeMode()` persists once on Done; (2) `SponsorScrollDirection` object added to KMP `Models.kt` (`LTR`, `RTL`, `TTB`, `BTT`, `FIXED`) with `sanitize()`, `isHorizontal()`, `isVertical()`; `OverlayLayoutPrefs` gains `sponsorScrollDirection` field (default `RTL`); `StreamCameraEngine.OverlayLayout` gains `sponsorScrollDirection`; `applySponsorSprite()` now branches on horizontal vs vertical axis — horizontal ticker animates X; vertical crawl animates Y; (3) Board-scale helpers added to KMP `OverlayLayoutPrefs`: `boardScale()` (uniform scale vs reference board), `withBoardScale(scale: Double)` (aspect-locked copy), `withAnchor(x, y)` (drag reposition clearing `bottomMargin`); `applyOverlaySprite()` in Android `StreamCameraEngine` now uses `OverlaySpriteLayout.fitScale()` + `computePosition()` driven by `widthFraction` as the single uniform board scale; (4) `OverlayLayoutPrefs.sanitizeTheme()` now accepts ONLY `"barlow"` — all other theme strings (classic/neon/minimal etc.) map to `"barlow"`; `OverlayWebViewCapture` CSS targets `body.board-barlow .sb-wrap` and `body.board-barlow .sb-bar`; (5) `StudioSheets.kt` `OverlaySheet` renamed to `Board Edit` — theme carousel removed; sponsor scroll direction picker (RTL/LTR/TTB/BTT/Fixed) added when sponsor scroll mode is active; board colour presets updated to Default/Dark/Black/Light/Teal; "◇ Arrange on screen" entry button added; (6) `BroadcastCameraUi` gains dedicated `LandscapeControls` composable (left rail = Dest/Board/Score tools, right rail = QuickToggles + Pause + shutter) mirroring the portrait bottom-stack layout — both layouts now contain identical tool, quick-toggle, and shutter sets; (7) `toEngineLayout()` extension in `RtmpCredentialsStore.kt` updated to pass `sponsorScrollDirection`; `bottomMarginPx()` extension still present.)*

*Confirmed in current code: iOS Go Live countdown is 3→1; Android `StreamRecap` (`durationSeconds`, `destinationLabel`, `watchUrl`) has no `title` field; `StreamOverlayPolicy.refreshMode` accepts `hasPreviewListener` for API compat but does not use it in the decision branch (result is always `None`/`StreamRefresh`/`PreviewGlRefresh`); Android `StudioViewModel` live-timer increments a plain counter (`elapsed++`); iOS `StudioViewModel` stamps `liveStartedAt: Date?` and computes wall-clock elapsed; poll cadences — Android matchDay=8 s, remote-commands=1.5 s; iOS matchDay=5 s, remote-commands=1.5 s; `OverlayLayoutPrefs.sanitizeTheme()` now only accepts `"barlow"` (Barlow is the sole board layout); `RELAY_PROVIDERS` contains only `"play_cricket"` and `"cricheroes"` — `relay_source="native"` has no scraper (manual-only); iOS `StreamCameraEngine.buildStandbyImage()` composes a Floodlight-branded full-frame slate shown while backgrounded; iOS `StreamCameraEngine` has `pauseBlackObject: ImageScreenObject?` + `showPauseBlackOverlay()` / `hidePauseBlackOverlay()`; `CRICHEROES_HOSTS = {"cricheroes.com", "cricheroes.in"}` + `_is_cricheroes_host()`; `canonicalize_cricheroes_scrape_url` returns `""` for non-CricHeroes URLs; `OverlayLayoutPrefs` carries `activeSponsorIds: List<String>` (multi-select) + `effectiveSponsorIds()` + `resolveSponsorLogoUrls()` + `mergeSponsorPatch()` + `sponsorPatchJson()` on all three platforms; KMP `OverlayLayoutPrefs` exposes `boardDisplayScaleX()` / `boardDisplayScaleY()` / `clampedWidthFraction()` / `clampedHeightFraction()` / `effectiveFontScale()` / `boardScale()` / `withBoardScale()` / `withAnchor()`; `OverlayLayoutPrefs` carries `videoStabilization` and `keepScreenOn` on all three platforms; `CompanionSession` struct in iOS `Models.swift`; `previewOverlayPrefs()` / `revertOverlayPreview()` on both platforms; iOS `cameraConfigQueue` serial DispatchQueue; `BroadcastCameraUi` `onShare` crossfades Menu→Share when streaming; `StreamOverlayPolicyTest` covers all branches; Android `StudioViewModel` lifecycle hooks `onStudioVisible()`, `onConfigurationChanged()`, `onDeviceOrientationChanged()`, `onStudioHidden()`.*
...
- Both layouts contain the identical tool, quick-toggle, and shutter sets — neither orientation hides controls behind the other.
