...
*Auto-maintained by Stop hook (`/.claude/hooks/update-architecture.sh`). Last updated: 2026-07-01 session 8 (re-validated all streaming pipeline files — no structural changes from session 7; all session-7 facts confirmed accurate vs. current code; two new KMP facts added: (1) `StreamMatch.fromJson()` resolves relative overlay embed URLs to absolute URLs via `resolveAbsoluteUrl(baseUrl, overlay)` when the path starts with `/` — the KMP client injects the base URL so mobile ViewModels never handle raw relative paths; (2) `ScoringConfig.localFallback(baseUrl, matchSlug, mode)` static factory constructs a full scoring-config object from local URL conventions when the server returns no scoring payload — used as an offline fallback in both platforms).*
