...
*Auto-maintained by Stop hook (`/.claude/hooks/update-architecture.sh`). Last updated: 2026-07-01 (public live-score page + SSE; public club page; SEO infrastructure; password reset flow; iOS OTA install; viewer-stat columns; ui_theme; scoring dual-write flags; stream slot cap; Sponsor time bounds; fixture Weekly-tab priority; relay_source_to_provider; relay_source="native" for club-owned tournament fixtures; tournament management dashboard routes + round-robin generation; public /t/<slug> tournament page; compute_points_table NRR standings; GDPR Art. 17 erasure + Art. 20 data-portability API endpoints; authenticated app-builds endpoint; home page (GET /) now passes app_builds payload (Android APK + iOS OTA links with streaming_note) to template — download links surfaced on public acquisition page; **CricHeroes dual-host refactor** — `CRICHEROES_HOSTS = {"cricheroes.com", "cricheroes.in"}` and `_is_cricheroes_host()` promoted to module-level constants in `models_cricrelay.py` as the single source of truth for host validation; `canonicalize_cricheroes_scrape_url` now returns `""` (falsy) for any URL that does not pass `_is_cricheroes_host` — previously returned raw input on failure, so callers now treat falsy return as "invalid URL" without their own string checks; netloc is lowercased in all canonical CricHeroes URLs; `_cricheroes_match_id_from_url` uses a two-pass regex (`/(?:scorecard|individual)/(\d+)` then `/scorecard/(\d+)/` fallback) and normalises `/individual/<id>/live` → `/scorecard/<id>/live`; `canonicalize_cricheroes_scrape_url` added to top-level imports in `app.py`; `relay_config()` calls `canonicalize_cricheroes_scrape_url` before validating and returns generic "CricHeroes scorecard page" error; `_create_cricheroes_stream_org` simplified to `if not full_url` guard (removes redundant `"cricheroes" not in full_url` string check); `normalize_cricheroes_team_root` uses `_is_cricheroes_host` and defaults to `cricheroes.com` (was `cricheroes.in`).*
