...
- `StreamCameraEngine.shared` — HaishinKit RTMP engine singleton (MediaMixer + RTMPStream, MTHKView preview, overlay WebView compositor, focus+AE lock via `cameraConfigQueue` serial queue, zoom, stabilisation, keep-screen-on, sponsor GL compositing, background standby slate, thermal monitoring)
