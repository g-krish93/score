...
*Auto-maintained by Stop hook (`/.claude/hooks/update-architecture.sh`). Last updated: 2026-07-01 session 6 (re-validated all streaming pipeline files — no structural changes from session 5; confirmed: Android `BroadcastCameraUi` pinch-to-zoom via `rememberTransformableState` + `onPinchZoom` callback; Android `StudioViewModel.onStudioHidden()` sets `previewSurfaceBound = false` and calls `streamController.hideNativePreview()` (complement to `onStudioVisible()`); all other documentation confirmed accurate vs. current code).*
