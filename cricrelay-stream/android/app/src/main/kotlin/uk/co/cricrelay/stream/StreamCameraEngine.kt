...
object StreamCameraEngine : ConnectChecker {
    ...
    internal val debugLogContext: Context?
        get() = appContext
    ...
    private fun startStreamOnMain(endpoint: String) {
        ...
        // #region agent log
        DebugAgentLog.log(
            location = "StreamCameraEngine.kt:startStreamOnMain",
            message = "startStreamOnMain entry",
            hypothesisId = "A,B,E",
            runId = "post-fix",
            data = mapOf(
                "encoderPrepared" to encoderPrepared,
                "isOnPreview" to cam.isOnPreview,
                "isStreaming" to cam.isStreaming,
            ),
        )
        // #endregion
        if (!encoderPrepared || !cam.isOnPreview) {
    ...
    private fun stopStreamInternal() {
        ...
        // #region agent log
        DebugAgentLog.log(
            location = "StreamCameraEngine.kt:stopStreamInternal",
            message = "stopStreamInternal finished",
            hypothesisId = "A,B",
            runId = "post-fix",
            data = mapOf(
                "encoderPrepared" to encoderPrepared,
                "isOnPreview" to (camera?.isOnPreview == true),
                "isStreaming" to (camera?.isStreaming == true),
                "isPreviewReady" to isPreviewReady,
            ),
        )
        // #endregion
    }
    ...
    override fun onConnectionFailed(reason: String) {
        pendingOverlayAfterConnect = false
        val msg = reason.ifBlank { "RTMP connection failed" }
        // #region agent log
        DebugAgentLog.log(
            location = "StreamCameraEngine.kt:onConnectionFailed",
            message = "RTMP connection failed",
            hypothesisId = "D",
            runId = "post-fix",
            data = mapOf("reason" to msg, "isStreaming" to isStreaming),
        )
        // #endregion
        emit(StreamCaptureService.EVENT_ERROR, msg)
    }
